...
export default function QuillEditor({ value, onChange, minHeight = '320px' }: QuillEditorProps) {
    ...
    (async () => {
      try {
        ...
        if (editorRef.current && !quillRef.current) {
          ...
          try {
            quillRef.current.root.style.minHeight = typeof minHeight === 'number' ? `${minHeight}px` : String(minHeight);
          } catch (e) {}
        }
      } catch (e) {
  ...
  return <div>
    <div ref={editorRef} style={{ minHeight: typeof minHeight === 'number' ? `${minHeight}px` : minHeight }} />
  </div>;
}
