...
> = ({
  ...
  initialTotalCount,
    ...
    columns.push({
      ...
      selector: (row) => (
        <button
          onClick={() => canEditMedicines && handleToggleStatus(row)}
          disabled={!canEditMedicines}
          className={`relative inline-flex items-center h-6 rounded-full w-11 transition-colors focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 ${!canEditMedicines ? "opacity-60 cursor-not-allowed" : "cursor-pointer"}`}
