...
function getDeleteStatus(isDelete?: boolean) {
    return isDelete ? (
        <span className="px-2 py-1 rounded-full bg-red-100 text-red-700 text-xs font-semibold">Deleted</span>
    ) : (
...
export default function AdminCustomerListPage() {
    ...
    useEffect(() => {
        setLoading(true);
        fetch(
            `/api/admin/customers/list?limit=${rowsPerPage}&offset=${page * rowsPerPage
            }&search=${searchTerm}`
        )
            .then((res) => res.json())
            .then((data) => {
        ...
        },
        {
            ...
            selector: (row) => (
                <>
                    ...
                    <span
                        className={`ml-2 text-xs font-mono ${row.isActive ? "text-green-700" : "text-gray-500"
                            }`}
                    >
                        {String(row.isActive)}
                    </span>
