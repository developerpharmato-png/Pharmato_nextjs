...
export default function OrderDetailPage() {
    ...
    try {
      ...
      if (data.success) {
        ...
        Swal.fire({
          icon: "error",
          title: "Error",
          text: data.message || "Failed to approve prescription",
        });
      }
    } catch (error) {
    ...
    try {
      ...
      if (data.success) {
        ...
        Swal.fire({
          icon: "error",
          title: "Error",
          text: data.message || "Failed to reject prescription",
        });
      }
    } catch (error) {
  ...
  const getPrescriptionStatusBadge = (status: string) => {
    ...
    if (statusLower === 'approved') {
      ...
      return <span className="px-3 py-1 rounded-full text-sm font-medium bg-orange-100 text-orange-800">Re-upload Required</span>;
    } else {
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <p className="font-medium text-gray-900 text-xs font-mono">{order.payment_id || "-"}</p>
            </div>
            <div>
        ...
        {order.userId && (
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <p className="text-sm text-gray-500">Registered Mobile Number</p>
                <p className="font-medium text-gray-900">
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="space-y-4">
              ...
              {order.prescription_url && (
                <div>
                  <p className="text-sm text-gray-500 mb-2">Uploaded Prescription</p>
              ...
              {order.prescription_rejection_reason && (
                <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                  ...
                  <p className="text-red-800">{order.prescription_rejection_reason}</p>
                </div>
              )}

              {order.prescription_approval_notes && (
                <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                  <p className="text-sm font-semibold text-green-900 mb-1">Approval Notes:</p>
                  <p className="text-green-800">{order.prescription_approval_notes}</p>
                </div>
              )}

              {order.prescription_approved_at && (
                <div className="text-sm text-gray-600">
                  <p>Approved on: {new Date(order.prescription_approved_at).toLocaleString("en-IN")}</p>
                </div>
              )}

              {order.prescription_rejected_at && (
                <div className="text-sm text-gray-600">
                  <p>Rejected on: {new Date(order.prescription_rejected_at).toLocaleString("en-IN")}</p>
                </div>
              )}
            </div>
          </div>
          ...
          {order.coupon_code && (
            <div className="mb-4 p-3 bg-green-50 border border-green-200 rounded-lg">
              <p className="text-sm text-green-800">
                <span className="font-semibold">Coupon Applied:</span> {order.coupon_code}
              </p>
            </div>
          )}
          <div className="space-y-4">
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium">
                {(order.delivery_charges !== undefined && order.delivery_charges > 0)
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl p-6 max-w-md w-full mx-4">
            ...
            <p className="text-sm text-gray-600 mb-4">
              You are about to approve this prescription. You can optionally add notes for your records.
            </p>
            <textarea
              value={approvalNotes}
      ...
      {showRejectModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl p-6 max-w-md w-full mx-4">
            ...
            <p className="text-sm text-gray-600 mb-4">
              Please provide a reason for rejecting this prescription. The customer will be notified and can re-upload a new prescription.
            </p>
            <textarea
