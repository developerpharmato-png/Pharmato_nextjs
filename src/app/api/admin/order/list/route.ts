...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        // Filter by order_status if provided (Sync with dashboard aggregation logic)
        if (order_status && typeof order_status === 'string' && order_status !== 'all') {
            const status = order_status.trim();
            if (status === 'Delivered') {
                query.order_status = { $regex: /deliv|complete|out for/i };
            } else if (status === 'Pending') {
                query.order_status = { $regex: /pending|upload/i };
            } else if (status === 'Confirmed') {
                query.order_status = { $regex: /confirm/i };
            } else if (status === 'Cancelled') {
                query.order_status = { $regex: /cancel|reject|return/i };
            } else if (status === 'Order Placed') {
                query.order_status = {
                    $regex: /placed|order/i,
                    $not: /deliv|out|complete|cancel|confirm/i
                };
            } else {
